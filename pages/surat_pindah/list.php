...
$result = mysqli_query($conn, "SELECT * FROM surat_pindah");
?>

<div class="flex justify-between">
    <div>
        <h2 class="text-3xl font-urbanist font-extrabold mb-2">Data Surat Pindah</h2>
        <p>Kelola data surat pindah yang tersimpan pada sistem sekolah.</p>
    </div>

    <!-- Button -->
    <div>
        <a href="pages/surat_pindah/add.php" class="group inline-flex items-center rounded-lg py-4 px-6 gap-1.5 text-sm text-white font-poppins font-medium bg-linear-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-[0_3px_4px_rgba(59,130,246,0.4)] transition duration-300">
    ...
    <table class="w-full text-sm text-left">
        <thead class="font-poppins font-medium bg-gray-100 text-sm text-gray-700 sticky top-0 z-1 shadow-md">
            <tr>
                ...
                <th scope="col" class="px-28 py-5 font-semibold text-gray-700">Tujuan Sekolah</th>
                <th scope="col" class="px-4 py-5 font-semibold text-gray-700">Alasan Pindah</th>
        ...
        <tbody class="divide-y divide-gray-200">
            ...
            ?>
                <tr class="bg-white hover:bg-gray-100 font-medium font-poppins transition text-sm">
                    ...
                    <td class="px-28 py-4 font-medium"><?= $row['sekolah_tujuan']; ?></td>
                    <td class="px-4 py-4 font-medium"><?= $row['alasan_pindah']; ?></td>
                    ...
                    <td class="px-4 py-4 relative flex justify-center">
                        ...
                        <div role="menu"
                            ...
                            <div>
                                <a href="pages/surat_pindah/edit.php?id_surat_pindah=<?= $row['id_surat_pindah']; ?>" class="block px-3 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 hover:text-gray-900" role="menuitem">Edit</a>
                            </div>
                            <a href="/poin_pelanggaran_siswa/process/surat_pindah/delete.php?id_surat_pindah=<?= $row['id_surat_pindah']; ?>" onclick="return confirm('Yakin ingin menghapus data ini?')" class="block w-full px-3 py-2 text-sm font-medium text-red-700 transition-colors hover:bg-red-50 ltr:text-left rtl:text-right">Hapus</a>
                        </div>
                    </td>
                </tr>
            <?php } ?>

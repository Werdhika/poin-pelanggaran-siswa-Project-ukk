...
$result = mysqli_query($conn, "SELECT * FROM ortu_wali");
?>

<div class="flex justify-between">
    <div>
        <h2 class="text-3xl font-urbanist font-bold mb-2">Data Orang Tua & Wali</h2>
        <p>Kelola data orang tua dan wali siswa yang tersimpan pada sistem sekolah.</p>
    </div>

    <!-- Button -->
    <div>
        <a href="pages/orang_tua/add.php" class="group inline-flex items-center rounded-lg py-4 px-6 gap-1.5 text-sm text-white font-poppins font-medium bg-linear-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-[0_3px_4px_rgba(59,130,246,0.4)] transition duration-300">
    ...
    <table class="w-full text-sm text-left">
        <thead class="font-poppins font-medium bg-gray-100 text-sm text-gray-700 sticky top-0 z-1 shadow-md">
            <tr>
                ...
                <th scope="col" class="px-4 py-5 font-semibold text-gray-700">Nama Ibu</th>
                <th scope="col" class="px-4 py-5 font-semibold text-gray-700">Nama Wali</th>
                <th scope="col" class="px-4 py-5 font-semibold text-gray-700">Kontak</th>
                <th scope="col" class="px-4 py-5 font-semibold text-gray-700">Aksi</th>
            </tr>
        </thead>
        ...
        <tbody class="divide-y divide-gray-200">
            ...
            ?>
                <tr class="bg-white hover:bg-gray-100 font-medium font-poppins transition text-sm">
                    ...
                    <td class="px-4 py-4"><?= !empty($row['ayah']) ? $row['ayah'] : '-'; ?></td>
                    <td class="px-4 py-4"><?= !empty($row['ibu']) ? $row['ibu'] : '-'; ?></td>
                    <td class="px-4 py-4 "><?= !empty($row['wali']) ? $row['wali'] : '-'; ?></td>
                    <td class="px-4 py-4"><?php
                                            if (!empty($row['no_telp_wali'])) {
                                                $nama_kontak = $row['wali'];
                                                $kontak = $row['no_telp_wali'];
                                            } elseif (!empty($row['no_telp_ayah'])) {
                                                $nama_kontak = $row['ayah'];
                                                $kontak = $row['no_telp_ayah'];
                                            } else {
                                                $nama_kontak = $row['ibu'];
                                                $kontak = $row['no_telp_ibu'];
                                            }
                                            ?>
                        <p class="text-[10px]"><?= $nama_kontak ?></p>
                    ...
                    <td class="px-4 py-4 relative flex justify-center">
                        ...
                        <div role="menu"
                            ...
                            <a href="/poin_pelanggaran_siswa/process/orang_tua/delete.php?id_ortu_wali=<?= $row['id_ortu_wali']; ?>" onclick="return confirm('Yakin ingin menghapus data ini?')" class="block w-full px-3 py-2 text-sm font-medium text-red-700 transition-colors hover:bg-red-50 ltr:text-left rtl:text-right">Hapus</a>
                        </div>
                    </td>
                </tr>
            <?php } ?>

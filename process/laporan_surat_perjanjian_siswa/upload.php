...
$targetDir = ROOTPATH . "/uploads/perjanjian_siswa/";
...
if (!empty($_FILES["foto_dokumen"]["name"])) {
    ...
    if (in_array($fileType, $allowTypes)) {
        if (move_uploaded_file($_FILES["foto_dokumen"]["tmp_name"], $targetFilePath)) {
            $query = mysqli_query($conn, "UPDATE perjanjian_siswa SET 
                foto_dokumen = '$fileName',
                status = 'Selesai'
                WHERE nis = '$nis'");

            if ($query) {
                header("Location: /poin_pelanggaran_siswa/pages/laporan/perjanjian_siswa/daftar_siswa/list.php");
                exit;
            } else {
